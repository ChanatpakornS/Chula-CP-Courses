...
object Q2 {
  def partialMap(l : List[String])(f1 : String => String)(f2 : String => Boolean): List[String] = {
    val x = partial(l,f1,f2,List())
    println(x)
    x
  }

  def partial(l: List[String], f1: String => String, f2: String => Boolean, ans: List[String]): List[String] = {
    if(l.isEmpty) ans
    else {
      if (f2(l.head)) partial(l.tail, f1, f2, ans ++ List(f1(l.head)))
      else partial(l.tail, f1,f2,ans ++ List(l.head))
    }
  }
