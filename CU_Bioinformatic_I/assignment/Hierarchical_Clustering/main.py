...
with open('Hierarchical Clustering.txt', 'r') as f:
    lines = f.readlines()
    data = [ line.strip() for line in lines]
    n = int(data[0])
    dist_matrix = np.array([ [ float(x) for x in line.split() ] for line in data[1:] ])
    print("n: ", n)
...
new_clusters_list = HierarchicalClustering(dist_matrix, 'average')
